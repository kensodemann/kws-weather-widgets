...
describe('kws-uv-index', () => {
  ...
  beforeEach(async () => {
    ...
    await page.setContent('<kws-uv-index></kws-uv-index>');
    element = await page.find('kws-uv-index');
  });
  ...
  it('displays the UV Index with a single sigfig', async () => {
    ...
    const value = await page.find('kws-uv-index >>> .value');
  ...
  describe('description', () => {
    it('is low for zero', async () => {
      element.setProperty('uvIndex', 0);
      await page.waitForChanges();
      const description = await page.find('kws-uv-index >>> .description');
  ...
  describe('class', () => {
    it('is low-risk for 0', async () => {
      element.setProperty('uvIndex', 0);
      await page.waitForChanges();
      const mainDiv = await page.find('kws-uv-index >>> div');
